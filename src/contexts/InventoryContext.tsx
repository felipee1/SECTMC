...
interface InventoryContextType {
  ...
  consumeItem: (uuid: string) => void;
  updateSettings: (settings: AppSettings) => void;
  getCategoryTotal: (category: Category) => number;
  getItemsByCategory: (category: Category) => InventoryItem[];
}
...
export function InventoryProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = useCallback((item: InventoryItem) => {
    ...
    setData((prev) => {
      ...
      day.produced = true;
      day.details = [...day.details, `Added ${item.quantity}x ${item.name}`];
      history[today] = day;
  ...
  const consumeItem = useCallback((uuid: string) => {
    ...
    setData((prev) => {
      ...
      day.consumed = true;
      if (consumed) {
        day.details = [...day.details, `Consumed 1x ${consumed.name}`];
      }
      history[today] = day;
